...
test('38 – class page shows correct header (title, education level, student count)', async ({
  teacherPage,
}) => {
  /**
...
test('39 – semester toggle defaults to Semestrul I on page load', async ({ teacherPage }) => {
  /**
...
test('40 – clicking semester II button switches the grade data view', async ({ teacherPage }) => {
  /**
...
test("41 – subject tabs render for the teacher's assigned subjects in class 2A", async ({
  teacherPage,
}) => {
  /**
...
test('42 – clicking the CLR subject tab loads student rows in the grade grid', async ({
  teacherPage,
}) => {
  /**
...
test('43 – back link returns the teacher to the dashboard', async ({ teacherPage }) => {
  /**
