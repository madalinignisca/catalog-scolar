...
test('57 – thesis grade is created or displayed with a distinguishing indicator', async ({
  teacherMiddlePage,
}) => {
  /**
...
test('58 – multiple grades for the same student are all visible in their row', async ({
  teacherMiddlePage,
}) => {
  /**
  ...
  for (let i = 0; i < Math.min(badgeCount, 3); i++) {
    ...
    if (boundingBox) {
      expect(boundingBox.width, `Badge at index ${String(i)} has zero width`).toBeGreaterThan(0);

      expect(boundingBox.height, `Badge at index ${String(i)} has zero height`).toBeGreaterThan(0);
    }
  }
});
...
test('59 – grade can be saved successfully without filling the optional description', async ({
  teacherPage,
}) => {
  /**
