...
test('44 – students are sorted alphabetically by last name in the grade grid', async ({
  teacherPage,
}) => {
  /**
...
test('45 – each student row has a student name and an add-grade button', async ({
  teacherPage,
}) => {
  /**
...
test('46 – qualifier grade badges (FB, B) are visible in class 2A CLR subject', async ({
  teacherPage,
}) => {
  /**
  ...
  const crisanRow = catalogPage
    .getStudentRowByName('Crișan')
    .or(catalogPage.getStudentRowByName('Crisan'));
...
test('47 – numeric grade badges are visible in class 6B ROM subject', async ({
  teacherMiddlePage,
}) => {
  /**
...
test('48 – hovering a grade badge shows a tooltip with the date', async ({ teacherPage }) => {
  /**
...
test('49 – thesis grade displays with a "T" prefix or thesis indicator for Alexandru Pop', async ({
  teacherMiddlePage,
}) => {
  /**
...
test('50 – loading skeleton is visible while the grades API response is delayed', async ({
  teacherPage,
}) => {
  /**
