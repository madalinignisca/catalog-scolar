...
authTest('66 – API 500 on grade fetch shows grade-grid-error element', async ({ teacherPage }) => {
  /**
   * SCENARIO
   * ────────
  ...
  await teacherPage.route('**/catalog/classes/*/subjects/*/grades*', (route) =>
    route.fulfill({
      status: 500,
...
authTest('67 – API 403 on grade creation shows an error message', async ({ teacherPage }) => {
...
test('68 – network timeout on login shows login-error element', async ({ page }) => {
  /**
   * SCENARIO
   * ────────
