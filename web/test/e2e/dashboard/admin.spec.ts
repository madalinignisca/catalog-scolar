...
test('24 – admin sees quick-access cards on the dashboard', async ({ adminPage }) => {
  /**
...
test('25 – clicking an admin card navigates to a different route', async ({ adminPage }) => {
  /**
  ...
  await adminPage.waitForFunction((before: string) => window.location.href !== before, urlBefore);
