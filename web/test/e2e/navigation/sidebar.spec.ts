...
test('30 – sidebar shows correct navigation items for the teacher role', async ({
  teacherPage,
}) => {
  /**
...
test('31 – active route is highlighted in the sidebar on the dashboard', async ({
  teacherPage,
}) => {
  /**
...
test('32 – sidebar footer shows the logged-in user name and role', async ({ teacherPage }) => {
  /**
  ...
  const nameIsCorrect = userName.includes('Ana Dumitrescu') || userName.includes('Ana');
...
test('33 – clicking the logout button ends the session and redirects to /login', async ({
  teacherPage,
}) => {
  /**
